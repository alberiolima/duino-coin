...
void waitForClientData(void) {
  ...
  while ((client.connected()) && (millis() < tF)) {
    if (client.available()) {
...
void loop() {
  ...
  unsigned int duco_numeric_result = 0;
  unsigned int aux_duco_numeric_result = 0;
  String aux_last_block = "";
  uint8_t unidade = 0;
  uint8_t hashArray[20];
  char teste_block_c[1] = {0};
  ...
  duco_numeric_result = 0;
  while ( duco_numeric_result < difficulty) {
    for (unidade = 0; unidade < 10; unidade++) {  
      // Difficulty loop
      teste_block_c[0] = '0' + unidade;
      sha1_ctx = sha1_ctx_base;
      br_sha1_update(&sha1_ctx, teste_block_c, 1);
      br_sha1_out(&sha1_ctx, hashArray);
      if (memcmp((const void *)hashArray, (const void *)bytes_expected_hash, 20) == 0) {                
        // If result is found
        unsigned long elapsed_time = micros() - start_time;
        duco_numeric_result += (unsigned long)unidade;
        float elapsed_time_s = elapsed_time * .000001f;
        hashrate = duco_numeric_result / elapsed_time_s;
        float hashrate_real = 0.0f;

        /* reduz velocidade para evitar receber dificuldade maior */
#ifdef MAX_HASHRATE
        if ( hashrate > MAX_HASHRATE ) {
          hashrate_real = hashrate;
          hashrate = MAX_HASHRATE - (float)random(0, 200);
        }
#endif
        share_count++;
    ...
    aux_duco_numeric_result++;
    aux_last_block = last_block_hash + String(aux_duco_numeric_result);
    br_sha1_init(&sha1_ctx_base);
    br_sha1_update(&sha1_ctx_base, aux_last_block.c_str(), aux_last_block.length());
    duco_numeric_result += 10;
  }
}

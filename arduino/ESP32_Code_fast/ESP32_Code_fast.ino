/* modified for high speed for Albério Lima (Brazil)
...
const char *WIFI_PASS = "My secret wifi pass";
...
void TaskMining(void *pvParameters) {
  ...
  for (;;) {
    ...
    while (jobClient.connected()) {
      ...
      br_sha1_update(&context, (const unsigned char *)hashUnderTest.c_str(), hashUnderTestLength);      
      
      unsigned char und[1] = {0};
      unsigned long nonceCalc = 0;
      unsigned long nonceCalcAux = 0;
      unsigned long startTime = micros();
      // Try to find the nonce which creates the expected hash
      while ( nonceCalc < TaskThreadData[taskId].difficulty ) {
        for (uint8_t unidade = 0; unidade < 10; unidade++) {
          // Define hash under Test
          und[0] = unidade + '0';  

          br_sha1_context context2 = context;
          br_sha1_update(&context2, (const unsigned char *)und, 1);
          br_sha1_out(&context2, shaResult);
          ...
          if ( memcmp( shaResult, expectedHashBytes, sizeof(shaResult) ) == 0 ) {
            ...
            float tempHashrate = (float)nonceCalc / elapsedTime;
            #ifdef MAX_HASHRATE
              if (tempHashrate < MAX_HASHRATE) {
                TaskThreadData[taskId].hashrate = tempHashrate;
              } else {
                TaskThreadData[taskId].hashrate = MAX_HASHRATE - (float)random(0, 500);
              }
            #endif

            nonceCalc += (unsigned long)unidade;
            ...
            if ( TaskThreadData[taskId].hashrate < 4000 && !ignoreHashrate) {
              ...
              // Print statistics
              Serial.println(String(taskCoreName + " retrieved job feedback: " + feedback + ", hashrate: " + (tempHashrate / 1000) + "kH/s, share #" + TaskThreadData[taskId].shares));
            }

            // Stop current loop and ask for a new job
            nonceCalc = TaskThreadData[taskId].difficulty;
            break;
          }
        }//for
        nonceCalcAux++;
        nonceCalc += 10;
        hashUnderTest = previousHash + String(nonceCalcAux);
